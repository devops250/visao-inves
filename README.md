...
Dashboard web para monitorar ligações da assistente de voz Sofia (ElevenLabs Conversational AI) — Visão Investimentos. Ligações antigas feitas via VAPI continuam sendo listadas enquanto o token VAPI estiver ativo.
...
2. Copiar `.env.local.example` para `.env.local` e preencher:
   ...
   # opcional — mantém histórico VAPI visível
   VAPI_API_TOKEN=...
   VAPI_ASSISTANT_ID=...
